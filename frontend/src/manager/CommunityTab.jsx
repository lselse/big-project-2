...
export default function CommunityTab() {
  ...
  return <section className="workspace-shell community-shell">
    ...
    {!isAdmin && <form className="data-panel form-panel community-form" onSubmit={savePost}>
      <div className="panel-heading"><div><h2>{editingId ? '게시글 수정' : '새 게시글'}</h2><p>질문은 답변이 완료되면 해결 상태로 변경할 수 있습니다.</p></div>{editingId ? <button type="button" className="secondary-button" onClick={reset}><X size={15} /> 취소</button> : <Plus size={20} />}</div>
    ...
    <div className="data-panel">
      <div className="panel-heading"><div><h2>게시글</h2><p>총 {filteredPosts.length}개의 게시글</p></div><label className="notice-search compact"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="제목·내용 검색" /></label></div>
    ...
    {selected && <div className="community-modal-backdrop" onClick={() => setSelected(null)}><article className="community-modal" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="community-actions">{!isAdmin && selected.canEdit && <button type="button" className="secondary-button" onClick={() => editPost(selected)}><Edit3 size={15} /> 수정</button>}{(isAdmin || selected.canEdit) && <button type="button" className="secondary-button danger" onClick={() => removePost(selected)}><Trash2 size={15} /> 삭제</button>}</div>
